...
colorStack.push(new THREE.Color(0xAAAA00));
...
sGL.prototype.run = function() {
    ...
    $(document).on('movedEvent', function(event, eData) {
        ...
        for(var i = 0; i < movedlist.length; i++) {
            ...
            else if( item.blockType == self.PUSHMATRIX ) {
                pushMatrix( CTM , STACK_CTM );
            }
            else if( item.blockType == self.POPMATRIX ) {
                CTM = popMatrix(STACK_CTM, self.scene);
            }
...
function drawBox(item, CTM, scene, rotateInfo) {
    ...
    var theta = rotateInfo.theta,
        rX = rotateInfo.rX,
        rY = rotateInfo.rY,
        rZ = rotateInfo.rZ;

    pivot.rotation.x = -theta*rX*Math.PI/180;
    pivot.rotation.y = -theta*rY*Math.PI/180;
    pivot.rotation.z = -theta*rZ*Math.PI/180;
...
function doRotate(rotateInfo, item) {
    rotateInfo.theta = item.data.t;
    rotateInfo.rX = item.data.x;
    rotateInfo.rY = item.data.y;
    rotateInfo.rZ = item.data.z;
}

function pushMatrix(CTM, stack_CTM){
    stack_CTM.push(CTM.clone());
    console.log(stack_CTM);
}

function popMatrix(stack_CTM, scene){
    var CTM = stack_CTM.pop();
    console.log(stack_CTM);
